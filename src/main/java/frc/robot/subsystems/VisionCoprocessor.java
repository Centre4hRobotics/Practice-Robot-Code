...
public class VisionCoprocessor extends Vision {
    ...
    public VisionCoprocessor() {
        cameraGlobalPose = visionTable.getDoubleArrayTopic("Global Pose").subscribe(new double[] {0, 0, 0});
        //tagToCameraPose = visionTable.getDoubleArrayTopic("Tag To Camera Pose").subscribe(new double[] {0, 0, 0});
    ...
    protected void createPose() {
        ...
        if (confident) {
            ...
            Pose3d cameraPose = new Pose3d(new Translation3d(pose[0], pose[1], pose[2]), new Rotation3d(pose[3], pose[4], pose[5]));
