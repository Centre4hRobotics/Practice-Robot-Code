...
public class VisionPhoton extends Vision {
    ...
    public VisionPhoton() {
        ...
        try {
            noTrenchTagLayout = new AprilTagFieldLayout(new File(Filesystem.getDeployDirectory().getPath() + "/no_trench_tags.json").getPath());
        } catch(Exception e) {
    ...
    protected void createPose() {
        ...
        if (!results.isEmpty())
        {
            ...
            long banned = result.getTargets().stream()
                .filter(x -> VisionConstants.bannedTags.contains(x.fiducialId))
                .count();
            tagCount -= banned;
            ...
            if (result.hasTargets())
            {
                ...
                tagIDs = result.getTargets().stream()
                    .mapToLong(t -> t.fiducialId)
                    .toArray();
